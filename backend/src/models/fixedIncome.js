...
const FixedIncome = {
  ...
  update: async (id, userId, incomeData) => {
    ...
    const query = `UPDATE fixed_incomes SET ${setClause} WHERE id = $${fields.length + 1} AND user_id = $${fields.length + 2} RETURNING id, description, amount, day, category_id AS \"categoryId\", start_date AS \"startDate\", end_date AS \"endDate\"`;
